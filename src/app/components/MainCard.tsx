    ...
    >
      <div className="p-3 sm:p-5">
        <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
          ...
          >
            <User className="w-5 h-5 sm:w-6 sm:h-6" style={{ color: trustColors.text }} />
          </div>

          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5 sm:gap-2">
              <h2 className="text-sm sm:text-lg font-bold text-white truncate">{username || 'Pioneer'}</h2>
              {isVip && (
                <span 
                  className="px-1.5 py-0.5 rounded text-[7px] sm:text-[8px] font-bold uppercase"
                  style={{ 
            ...
            <p className="text-[9px] sm:text-[11px] font-mono text-gray-500 truncate">{formatAddress(walletAddress)}</p>
          </div>
          
          {onShare && (
            <button
              onClick={onShare}
              className="p-2 sm:p-2.5 rounded-xl transition-all active:scale-95 shrink-0"
              style={{
        ...
        <div 
          className="flex items-center justify-between p-2.5 sm:p-3 rounded-xl mb-3 sm:mb-4"
          style={{
        ...
        >
          <div className="flex items-center gap-2">
            ...
            >
              <span className="text-cyan-400 font-bold text-base sm:text-lg">π</span>
            </div>
            <div>
              <p className="text-[8px] sm:text-[10px] text-gray-400 uppercase font-medium">Wallet Balance</p>
              <p className="text-lg sm:text-xl font-black text-cyan-400">
                {balance !== undefined ? balance.toFixed(2) : '--'} <span className="text-sm">π</span>
              </p>
            </div>
          </div>
          <div className="text-right">
            <p className="text-[8px] sm:text-[9px] text-gray-500 uppercase">Network</p>
            <p className="text-xs sm:text-sm font-bold text-purple-400">Pi Mainnet</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 sm:gap-3 mb-4 sm:mb-5">
          ...
          >
            <p className="text-xl sm:text-3xl font-black" style={{ color: trustColors.text }}>
              {reputaScore}
            </p>
            <div className="flex items-center justify-center gap-0.5 sm:gap-1 mt-0.5 sm:mt-1">
              <span className="text-[8px] sm:text-[9px] uppercase font-bold text-gray-400">Reputa</span>
              <span 
            ...
            <p className="text-[8px] sm:text-[9px] uppercase font-bold text-gray-400">Score</p>
          </div>

          <div 
            className="p-2 sm:p-3 rounded-xl text-center flex flex-col items-center justify-center"
            style={{
              background: 'linear-gradient(145deg, rgba(139, 92, 246, 0.15) 0%, rgba(0, 0, 0, 0.4) 100%)',
              border: '1px solid rgba(139, 92, 246, 0.3)',
            }}
          >
            <p className="text-xl sm:text-3xl font-black text-purple-400">{level}</p>
            <p className="text-[8px] sm:text-[9px] uppercase font-bold text-gray-400 mt-0.5 sm:mt-1">Level</p>
          </div>
          ...
          >
            <div 
              className="px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-lg mb-0.5 sm:mb-1"
              style={{ background: trustColors.bg, border: `1px solid ${trustColors.border}` }}
            >
              <p className="text-[10px] sm:text-sm font-black uppercase whitespace-nowrap" style={{ color: trustColors.text }}>
                {trustInfo.label}
              </p>
            </div>
            <p className="text-[8px] sm:text-[9px] uppercase font-bold text-gray-400">Trust Rank</p>
          </div>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5 sm:mb-2">
            <div className="flex items-center gap-1.5 sm:gap-2">
              <span className="text-[10px] sm:text-xs font-bold uppercase" style={{ color: trustColors.text }}>
                {trustInfo.label}
              </span>
              <TrendingUp className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-gray-500" />
              <span className="text-[10px] sm:text-xs text-gray-400">{trustInfo.nextLabel}</span>
            </div>
            <span className="text-[10px] sm:text-xs font-bold" style={{ color: trustColors.text }}>
